...
| A-009 | Gallery、作品素材与版权边界 | 需验证 | 采用策展准入门、评分卡与章节编排流程；首批资产和权利台账待验证 |
...
- `M1-CANDIDATE-10`：由 `bead-gen` 产出的约 10 幅完整 2D 候选 Artwork；用于 Museum 1 策展、导入、画境与谜题可行性评估，不自动成为首发清单。
- `VS-PUZZLE-10`：当前项目从候选画作中定义的约 10 个 Fragment / PuzzleDefinition 技术样本；推荐用四幅测试 Artwork 覆盖 1、2、3、4 Fragment cardinality 和最后一题完成边界。

二者均不代表首发规模，且真实破损原则优先于凑足测试分布。
...
详细流程以 [`CONTENT_CURATION.md`](CONTENT_CURATION.md) 为唯一主规范：

1. 先按主题、世界观阶段、权益与版权边界确认 Museum；
2. 再按叙事岗位、难度带、画境模式和组合节奏把 Artwork 放入 Gallery（Chapter 只通过 `chapterNarrativeID` 映射）；
3. 最后由当前项目定义 1–4 个真实破损、Nonogram 与恢复反馈。

每幅入章作品必须有一个主要叙事岗位。候选阶段先过权利、无损资产、破损可行性与 Museum/初步 3D 适配门，再使用 100 分策展卡排序；Fragment、谜题与世界包生产后，必须追加通过唯一纯逻辑解、恢复辨识、性能与无障碍发布门。初始阈值为 75 分进入标准槽位、85 分竞争英雄/高潮槽位；权重与阈值必须由首批资产和垂直切片校准，发布门不因总分高而豁免。

### 资产与项目职责

- `bead-gen` 只交付完整 2D 拼豆网格、预览、调色板、来源与 hash。
- 当前项目负责章节编排、3D 画境、破损、Nonogram、solver、恢复反馈、内容包和 QA。
- App 不在运行时依赖 `bead-gen`。
- `M1-CANDIDATE-10` 与 `VS-PUZZLE-10` 的定义见 A-005，不能混称“10 个素材”。

### Museum 1 候选编排

约 10 幅候选作品先按 `3 / 3 / 4` 容量进入三个 Gallery 的预编排。叙事扩展转正前只验证概念无关的功能节奏：Gallery 1 以单焦点、5×5/10×10 和现实修复/短暂回声为主；Gallery 2 以可导航空间、10×10/15×15 和一幅有限 3D 英雄作品为主；Gallery 3 以关系/历史后果、15×15/少量 20×20 和跨层恢复反馈为主。若 `NARRATIVE_EXPANSION.md` 获批，再分别映射到“无完整画桥”“第一次进入画境”“系统性白蚀证据”等叙事专名。该容量与映射都不是首发承诺，不适配的作品必须返回候选池而非强塞。

### 验证项目

- 策展评分能否预测叙事理解、完成率和生产成本；
- `3 / 3 / 4` 预编排是否形成合理难度波形；
- 作品权利与来源台账是否完整；
- 一幅有限 3D 英雄作品能否达到移动端性能与无障碍门槛；
- 候选画作能否自然产生真实、唯一解的 Fragment，而非为测试凑数；
- Gallery 3 的 20×20 / 25×25 上限。
...
5. A-009：用 `M1-CANDIDATE-10` 和英雄画境验证策展流程，再锁定三个 Galleries 的正式素材与权利台账。
