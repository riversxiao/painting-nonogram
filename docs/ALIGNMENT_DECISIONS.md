...
| A-012 | 世界观与 Museum 1 叙事边界 | 已变更 | 历史决定；由 A-013 supersede，不再作为当前 Canon |
| A-013 | 画桥前史、重启时间线与 Story milestones | 已对齐 | 灾前画桥、第一代清除、灾后重启链与独立 Story milestones 固定 |
...
- 推进 Museum。
- 推进故事。
- 触发、满足或完成任何 Story milestone，或发布以 entitlement / Blueprint access 为来源的叙事触发事件。
- 授予亲手修复印章。
...
正式内容不使用预填格，并必须在无预填条件下仅凭 clues 和标准彩色 Nonogram 规则具有唯一解、可由版本化纯逻辑 solver 完成。`5×5` 教学、演示和无障碍辅助可以使用预填格；预填属于具体游玩 Session 的辅助层，schema 必须明确记录来源。任何使用预填格的完成都不计入 `completedWithoutHints`。
...
> 变更说明：以下内容保留为决策历史，不再作为当前 Canon；旧画桥时序已由 A-013 取代。
...
## A-013：画桥前史、重启时间线与 Story milestones

- **状态：已对齐**
- **日期：2026-09-02**
- **Supersedes：A-012**
- **影响范围：故事圣经、Gallery/Chapter 编排、角色、画境、英雄画作、档案文案、状态模型、entitlement resolver 与后续 Museum**

### 正式 Canon

- 画桥正式名为“绘画认知回溯与接驳系统”，由长夜前的科学家发明；当时团队误以为它是神经模拟/历史重构技术。
- 第一代入画者实际进入真实画境后，被白蚀背后的外星力量发现并杀死，现实身体同步死亡。研究团队、事故记录以及技术真正意义随后遭到语义消除。
- 机器实体留在博物馆地下，被误分类为用途未知的旧扫描设备；文明断层后，幸存者失去操作它所需的知识链。
- 第一章六幅 Artwork 的修复只恢复“研究员身份 → 接驳锚点 → 地下实验室位置 → 校准/维护记录”这一条窄技术知识链。章末主角找到机器；A 的程序性身体记忆只支持数秒短启动/回声，不构成完整进入。
- 第二章是当前团队重新校准并重启既有画桥的阶段。此处完成的是灾后首次、也是主角首次完整进入，不是画桥的当代发明，不是设备首次投入使用，也不是人类历史上的首次进入。主角不是首位入画者，文明修复署不是画桥开发者。
- Museum 1 结尾证明外星力量在长夜前已主动清除第一代计划，并识别到一个本应被删除的“新操作员”重启了同一座门。结尾不揭示对方完整外形、社会或最终目标，玩法仍为修复而非战斗。
- B 表示主角此前被外星系统判定为“已清除”，因此未被常规追踪；画桥重启产生新的操作员特征，使这一隐身状态失效。不得把 B 写成永久无法追踪或永久认知免疫。
- 画桥关键原理是否来自对白蚀的逆向研究仍为开放细节；画桥灾前发明、第一代计划及其遭主动清除事件已经冻结。

### 独立 Story milestones

以下里程碑使用稳定 `storyMilestoneID`，与 Puzzle、Fragment、Artwork、Gallery/Chapter、Museum、Blueprint access 和 Restorer seal 分开记录：

| storyMilestoneID | 达成含义 |
|---|---|
| `technicalChainRestored` | 第一章六幅修复已恢复完整窄技术知识链 |
| `bridgeLocated` | 主角已按恢复线索找到地下画桥实体 |
| `bridgeBrieflyStarted` | 主角凭 A 完成数秒短启动/回声，且未完整进入 |
| `bridgeRebooted` | 当前团队已在 A12 完整接驳 session 开放前完成既有画桥的重新校准与灾后重启 |
| `firstPostCollapseFullEntryCompleted` | 主角已在 `bridgeRebooted` 后完成 A12 同一 session，达成灾后首次完整进入 |
| `firstGenerationPurgeRevealed` | 已用非正面证据确认第一代入画者在画境遇害、现实身体同步死亡，并证明第一代计划在灾前遭外星力量主动清除 |
| `newOperatorDetected` | 已证明重启产生新操作员特征并使 B 的隐身状态失效 |

Museum entitlement 只能改变 Blueprint access。购买、授予、恢复、查看或解析 entitlement 均不得触发、满足或完成上述任一 Story milestone，也不得通过发布叙事事件间接推进它们。里程碑只能由内容规范显式映射的实际玩法/叙事事件触发。

### 变更原因

A-012 把当前主线的第二章节点概括成全局画桥时序，无法表达灾前发明、第一代计划与清除事件，也容易把开发者身份错误归于修复署，或让主角被误读为历史上首位入画者。A-013 保留白蚀、真实画境、A+B 与 Museum 1 揭示上限，同时以灾前前史、灾后重启链和独立状态合同取代旧时间线。
...
4. 玩家进度迁移政策：确认 revision 变化后进行中局与已完成记录的具体处理；当前只有“禁止静默重解释旧状态”是硬边界。
5. 在 V1 数据与质量门稳定后，规划 CloudKit V1.1 与可选分页 PDF。
