...
- 首次启动先完成共同故事背景，再进入 `5×5` 共同教学。
- `5×5` 教学无计时和失败惩罚，完整教会 Filled、Excluded 与行列线索。
- “跳过教学”清楚可见；完成或明确跳过前，拼豆工坊和路径选择不开放。
- 跳过后可继续选择任一路径，且之后能从帮助中重玩教学。
- 学徒成长与专业画师在选择页面和首页层级平等。
- 选择只改变后续引导和默认落点，不锁定功能。
- 修复室与工坊自由切换。
- 中途退出和恢复。
- 完成作品后蓝图出现。
- 购买蓝图后谜题仍未完成。
...
- App Store 第一张截图主要展示修复游戏：受损作品、Nonogram 棋盘与局部恢复结果。
- 后续截图展示共同故事、两条平等路径和专业画师工坊。
- 第一张截图的沟通顺序不改变 App 内两条路径的平等层级。
...
通过：新用户先理解共同故事，并通过完成或明确跳过 `5×5` 教学理解基本玩法；随后平等选择学徒成长或专业画师。选择只影响后续引导，之后能在同一 App 中自由切换并共享作品。
